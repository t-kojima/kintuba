...
module.exports = class RecordEventObject extends EventObject {
  ...
  rollbackDisallowFields() {
    ...
    const isDisallows = key =>
      EDIT_DISALLOWS.some(a => a === schema.fields.properties[key].type);
    ...
    const isLookupMapping = key => {
      const mappings = Object.keys(schema.fields.properties)
        .filter(
          k =>
            schema.fields.properties[k].lookup &&
            schema.fields.properties[k].lookup.fieldMappings
        )
    ...
    const hasSchema = key =>
      schema.fields.properties
        ? Object.keys(schema.fields.properties).some(a => a === key)
        : false;
