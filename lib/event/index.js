...
const app = {
  record: {
    index: {
      ...
      edit: {
        ...
        submit: (event, options) =>
          new RecordEditSubmitEventObject(event, options),
        change: {},
      },
      delete: {
    ...
    detail: {
      ...
      process: {
        proceed: (event, options) =>
          new RecordProcessEventObject(event, options),
      },
    },
    create: {
...
const func = event => {
  ...
  return items.reduce((acc, cur) => (acc ? acc[cur] : app), undefined);
};

// fields項目の関数を定義する
const appendFieldChangeEvent = event => {
  const match = event.match(
    /^(app\.record\.(index\.edit|edit|create)\.change)\.([^.]+)$/
  );
  if (!match) return;
  const key = match[3];
...
const removeFieldChangeEvent = event => {
  if (event) {
    ...
    if (!match) return;
    const key = match[3];
    if (typeof func(match[1])[key] === 'function') {
      func(match[1])[key] = {};
    }
  } else {
...
const validate = event => {
  if (
    !event.match(
      /^app\.(record|report)(\.(index|detail))?(\.(create|edit|delete|print))?(\.(show|change|submit|process))?(\.(success|proceed))?(\..+)?$/
    )
  ) {
...
module.exports = class Event extends EventEmitter {
  ...
  async do(event, options) {
    ...
    const eventObj = func(event)(event, options);
    // kintone.appへ変更を通知
